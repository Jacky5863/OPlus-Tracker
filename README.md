...
Collection of tools for querying and resolving OTA / SOTA / OPEX / IOT update links for OPPO, OnePlus, Realme devices (ColorOS / OxygenOS).
...
- `iot_query.py`       → legacy & IoT / iota.coloros.com special server query (CN only)

## `C16_transer.py`

### Features
- Resolve dynamic links with using `downloadCheck?`
...
Often returns older or special builds that are no longer available through normal channels.

### Features
- Auto suffix completion for simple inputs (`_11.A`, `_11.C`, etc.)
- Very simple output: link, changelog, security patch, version
...
**Note**:  
- Only supports region `cn`  
- This server is for **legacy** and for **IoT** devices — results may be outdated or inconsistent compared to `tomboy_pro.py`
...
- `opex_query.py`, `sota_query.py` and `iot_query.py` are **CN-only** at the moment.
